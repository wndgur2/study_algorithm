...
int main(){
    ...
    while(left < right){
        ...
        // cout << "GN: " << groupN << endl;
        if(groupN <= M){
            // 만들수있는 그룹 수가 너무 적으면 right을 mid로 (최대 그룹합을 더 줄인다.)
            // 같아도 줄이는 이유는 그중 최소값을 찾기위해
            right = mid;
        } else{
            // 그룹이 충분히 많다면 left를 mid로 (최대 그룹합을 늘린다.)
            left = mid+1;
        }
    }
    cout << right << endl;

    // mid를 최대 그룹합으로 하는 그룹을 짓는다.
    ...
    for(i=0; i<N; ++i){
        ...
        // 남은 구슬이 남은 그룹 개수이면 나가서 나머지 1로 채움
